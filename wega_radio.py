...
class WegaRadioControl(object):
    ...
    def __init__(self, mpd_client, gpio_client):
        self.log = logging.getLogger("WegaRadioControl")
        self.mpdClient = mpd_client
        self.gpioClient = gpio_client
        self.stations = dict()

        # register callback to switch the radio off
        self.gpioClient.add_input_channel_callback(self.SWITCH_OFF_GPIO_CHANNEL, GPIO.FALLING, self._switch_off_callback)

    def setup_stations(self, stations):
...
if __name__ == '__main__':
    ...
    radioControl = WegaRadioControl(musicClient, gpioClient)
    radioControl.setup_stations(config['stations'])
    try:
        while True:
